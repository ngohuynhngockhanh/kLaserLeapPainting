...
.factory('Fabric', [
	...
	return function(options) {
		...
		// Shape
		// ==============================================================
		self.addShape = function(svgURL, onlyFirstShape) {
			fabric.loadSVGFromURL(svgURL, function(objects) {
				var object = (onlyFirstShape) ? objects[0] : fabric.util.groupSVGElements(objects, self.shapeDefaults);
				object.id = self.createId();
				for (var p in self.shapeDefaults) {
					object[p] = self.shapeDefaults[p];
				}
				
				object.setFill('white');
